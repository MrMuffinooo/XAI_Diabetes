...
*Authors: Martyna Majchrzak, Jakub Jung, Karol Niewiadowski (Warsaw University of Techcnology)*
...
In this article we attempt to find the relationship between different biomarkers and positive diagnosis of diabetes based on the data of over 700 women of Indian ancestry (Pima tribe). This dataset is originally from the National Institute of Diabetes and Digestive and Kidney Diseases. In order to do that, we used XAI tools on our black box models to present the decision making process of algorithms in understandable for humans way from which we could later draw conclusions.
...
In this section we will take time to investigate some model explanations that we will be using later. Feel free to use this chapter as reference for quick overview. For more in-depth knowledge please refer to paper [@2-4-XAI] All images in this section are also taken from this article.
Explanations themselves were implemented in the DALEX package [@2-4-DALEX].
...
```{r 2-4-break-down, fig.align='center',fig.cap='Panel A shows the distribution and mean prediction with values on the same level and above fixed on values from observation. Then change in mean is calculated and assigned to corresponding variables',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-break_down_distr.png')
```


A big drawback for this method is how the order of variable can influence the explanation outcome. This problem occurs most often as a result of existing interactions between variables as seen in (\@ref(fig:2-4-ordering))

```{r 2-4-ordering, fig.align='center',fig.cap='Variables age and class interact with each other so the changing order results in different plots',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r 2-4-shap-ordering, fig.align='center',fig.cap='Shap output. Average impact calculated with Break Down for 10 permutations',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
Ceteris Paribus is a Latin phrase meaning "all else unchanged". This accurately describes what it does. For the certain observation we take values that are interesting to us and observe how the prediction changes as we change values in those columns one at a time. This can yield interesting conclusions about change in values and its impact on predictions. Figures (\@ref(fig:2-4-titanicCeterisProfile01E-1)) and (\@ref(fig: 2-4-titanicCeterisProfile01D-1)) show prediction change as response for variable change.

```{r 2-4-titanicCeterisProfile01E-1, fig.align='center',fig.cap='Shape of lines resembles the nature of the model. Dot shows value and models prediction for the observation', out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-titanicCeterisProfile01E-1.png')
```

```{r 2-4-titanicCeterisProfile01D-1, fig.align='center',fig.cap=' Ceteris Paribus shows different importance in variables and their changes for different observations.',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r 2-4-TitanicRFFeatImp, fig.align='center',fig.cap=' Feature Importance output for 10 permutations',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-TitanicRFFeatImp10-1.png')
```

**Partial Dependence**

To put it short, Partial Dependence is the average of Ceteris Paribus for whole data. This results in average importance and impact of variable and its values. The similarities can be observed in figure (\@ref(fig:2-4-pdpIntuition-1))

```{r 2-4-pdpIntuition-1, fig.align='center',fig.cap='Multiple observations with Ceteris Paribus and their average as Partial Dependence',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r 2-4-pdpPart5-1, fig.align='center',fig.cap='Partial Dependence grouped by sex',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
Table: (\#tab:dataset-comparison) The value of performance measures AUC, Balanced Accuracy and False Negative Rate on dataset versions: original, naomit, mice and skip.
...
Table: (\#tab:dataset-original) The value of performance measures AUC, Balanced Accuracy and False Negative Rate on original dataset version using 6 classification models.
...
Table: (\#tab:dataset-skip) The value of performance measures AUC, Balanced Accuracy and False Negative Rate on dataset version with skipped columns, using 6 classification models.
...
Based on the results, two models: Ranger and Ada were chosen for further experiments. Hyperparameter tuning using random grid search and FNR as a minimalized measure was performed, resulting in classificators with the following values:
...
Table: (\#tab:dataset-tuned) The value of performance measures AUC, Balanced Accuracy and False Negative Rate on 2 dataset versions: original and with skipped columns, using tuned classification models Ranger and Ada.
...
```{r 2-4-feature-importance, fig.align='center',fig.cap='Variable importance for Ada and Ranger based on AUC loss after permutations.',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r 2-4-pdp, fig.align='center',fig.cap='Average predictions with PDP for ada and ranger.',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
Accumulated Dependence gave us similar results to Partial Dependence, thus we can conclude that our dataset does not contain any problematic correlations. Comparison between PD and AD is plotted on the Figure (\@ref(fig:2-4-pdp-ale-mass)) for _mass_ variable.

```{r 2-4-pdp-ale-mass, fig.align='center',fig.cap='Average predictions with PDP and ALE for ada and ranger.',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
Ceteris Paribus shows us results adequate to Pratial Dependence Profile (See Figures (\@ref(fig:2-4-ceteris-paribus)) and (\@ref(fig:2-4-lime_190))). Predictions are very consistent in variable _plas_. However in _age_ and _pedi_ cases we can spot different trends in prediction behaviour. This could hint towards potential interactions between variables. As for _mass_ and _preg_ they differ mostly in initial prediction value. Overall tendency is rising.


```{r 2-4-ceteris-paribus, fig.align='center',fig.cap='Ceteris Paribus predictions for five selected observations.',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
```{r 2-4-lime_190, fig.align='center',fig.cap='Lime output for observation 190',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-lime_190.png')
```


Breakdown and Shap (Figures (\@ref(fig:2-4-bd-190)) and (\@ref(fig:2-4-shap_190))) as opposed to Lime give us prediction change for exact values, not intervals. Because of that the results may be different. So is the case with the _age_ variable which behaves differently for Lime. According to Breakdown and Shap being 29 years of age slightly decreases prediction but Lime says otherwise. Not only it increases but also by a significant amount. Other variables seem to match their impact if just scaled down a bit for BD and Shap.

```{r 2-4-bd-190, fig.align='center',fig.cap='Breakdown output for observation 190',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-bd_190.png')
```

```{r 2-4-shap-190, fig.align='center',fig.cap='Shap output for observation 190',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-shap_190.png')
```

The next observation we analyze is observation 214 for which the model has a prediction of about 44% for positive diagnosis of diabetes. Lime explanation (Figure (\@ref(fig:2-4-lime_214))) tells us that three biggest factors for this prediction are _pedi_, _plas_ and _mass_. This time the first one is _pedi_ with high value of over 0.5. Despite high values of _plas_ (over 115) and _mass_ (over 33), the prediction is higher than average for this dataset, but still lower than 50%.

```{r 2-4-lime_214, fig.align='center',fig.cap='Lime output for observation 214',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-lime_214.png')
```


Breakdown and Shap (Figures (\@ref(fig:2-4-bd-214)) and (\@ref(fig:2-4-shap-214))) show similar results, just toned down. The biggest difference is the impact of _pedi_ which in Lime was nearly twice as big as the impact of _plas_ and _mass_. The only value making it less probable that the observation would have diabetes is _age_ equal to 28.

```{r 2-4-bd-214, fig.align='center',fig.cap='Breakdown output for observation 214',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
knitr::include_graphics('images/2-4-bd_214.png')
```

```{r 2-4-shap-214, fig.align='center',fig.cap='Shap output for observation 214',out.width="100%", echo=FALSE, warning=FALSE, message=FALSE}
...
Since all the women in the dataset have the same **ethnicity**, it is not a factor that can be considered valuable in this experiment. Since American Indians are more prone to diabetes, the results of studies on this dataset should not be invoked when diagnozing patients of other ethnicities.

### Conclusions

The investigation of Diabetes dataset resulted in:
- 
