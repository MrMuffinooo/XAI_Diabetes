...
Table: (\#tab:dataset-comparison) The value of performance measures AUC, Balanced Accuracy and False Negative Rate on dataset versions: original, naomit, mice and skip. For measures AUC and BAC higher value indicates better model, while for FNR - worse model.
...
The performance measures of tuned models are higher than the original ones, but ranger still proves to be a better classificator on this dataset.
...
Moving forward we tested a number of popular Machine Learning models on our data. Measures like AUC, BAC and FNR showed us that Ranger and Ada give us sufficient results to draw conclusions from. 
