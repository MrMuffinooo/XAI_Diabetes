...
for(i in 1:4){
  ...
  FNR[i] <- measureFNR(prediction$truth, 
                       ...
                       negative="0",
                       positive = "1")
  BACC[i]<-measureBAC(prediction$truth, 
                       prediction$response)
...
```{r results}
results<-data.frame(AUC=auc, FalseNegativeRate=FNR, BalancedAccuracy=BACC)
...
classif_lrn_ada <- makeLearner("classif.ada", predict.type = "prob")
model_ada <- mlr::train(classif_lrn_ada, classif_task)

classif_lrn_binomial <- makeLearner("classif.binomial", predict.type = "prob")
model_binomial <- mlr::train(classif_lrn_binomial, classif_task)

classif_lrn_gbm <- makeLearner("classif.randomForest", predict.type = "prob")
model_gbm <- mlr::train(classif_lrn_gbm, classif_task)

classif_lrn_naiveBayes <- makeLearner("classif.naiveBayes", predict.type = "prob")
model_naiveBayes <- mlr::train(classif_lrn_naiveBayes, classif_task)

classif_lrn_rForest <- makeLearner("classif.randomForest", predict.type = "prob")
model_rForest <- mlr::train(classif_lrn_rForest, classif_task)

pred_test <- predict(model_ada, newdata = diabetes_test)$data

auc[1] <- measureAUC(probabilities = pred_test$prob.1,
                       truth = pred_test$truth,
                       negative="0",
                       positive = "1")
  
FNR[1] <- measureFNR(pred_test$truth, 
                       pred_test$response,
                       negative="0",
                       positive = "1")
...
pred_test <- predict(model_binomial, newdata = diabetes_test)$data

auc[2] <- measureAUC(probabilities = pred_test$prob.1,
                       truth = pred_test$truth,
                       negative="0",
                       positive = "1")
  
FNR[2] <- measureFNR(pred_test$truth, 
                       pred_test$response,
                       negative="0",
                       positive = "1")


pred_test <- predict(model_gbm, newdata = diabetes_test)$data

auc[3] <- measureAUC(probabilities = pred_test$prob.1,
                       truth = pred_test$truth,
                       negative="0",
                       positive = "1")
  
FNR[3] <- measureFNR(pred_test$truth, 
                       pred_test$response,
                       negative="0",
                       positive = "1")


pred_test <- predict(model_naiveBayes, newdata = diabetes_test)$data

auc[4] <- measureAUC(probabilities = pred_test$prob.1,
                       truth = pred_test$truth,
                       negative="0",
                       positive = "1")
  
FNR[4] <- measureFNR(pred_test$truth, 
                       pred_test$response,
                       negative="0",
                       positive = "1")


pred_test <- predict(model_rForest, newdata = diabetes_test)$data

auc[5] <- measureAUC(probabilities = pred_test$prob.1,
                       truth = pred_test$truth,
                       negative="0",
                       positive = "1")
  
FNR[5] <- measureFNR(pred_test$truth, 
                       pred_test$response,
                       negative="0",
                       positive = "1")

res<-data.frame(AUC=auc, FalseNegativeRate=FNR)
row.names(res)<-c("Ada", "Binomial", "GBM","Naive Bayes","Random forest")
res
