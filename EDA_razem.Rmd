...
## Tuning params Ranger - skip

Użyjemy random search dla hiperparametrów: `num.trees`, `mtry`, `min.nod.size` i `splitrule`. 
Użyjemy zbiorów danych diabetes_skip i posłużymy się miarą fnr.

```{r}
classif_task <- makeClassifTask(id = "lvr_tune_random", data = diabetes_train_skip, target = "class")
classif_lrn <- makeLearner("classif.ranger", predict.type = "prob")
cv <- makeResampleDesc("CV", iter = 10)
model_raw <- mlr::train(classif_lrn, classif_task)
pred_test <- predict(model, newdata = diabetes_test)$data$prob.0
...
```
```{r}
model_random_skip <- train(res_random$learner, classif_task)
pred_random <- predict(model_random_skip, newdata = diabetes_test_skip)$data
fnr_ranger_tuned_skip<-measureFNR(pred_random$truth,
                       pred_random$response,
                       negative="0",
                       positive = "1")
fnr_ranger_tuned_skip

```
## Tuning params Ranger - original

Użyjemy random search dla hiperparametrów: `num.trees`, `mtry`, `min.nod.size` i `splitrule`. 
Użyjemy oryginalnych zbiorów danych diabetes i posłużymy się miarą fnr.

```{r}
classif_task <- makeClassifTask(id = "lvr_tune_random", data = diabetes_train, target = "class")
classif_lrn <- makeLearner("classif.ranger", predict.type = "prob")
cv <- makeResampleDesc("CV", iter = 10)
model_raw <- mlr::train(classif_lrn, classif_task)
pred_test <- predict(model, newdata = diabetes_test)$data$prob.0
...
model_random <- train(res_random$learner, classif_task)
pred_random <- predict(model_random, newdata = diabetes_test)$data
fnr_ranger_tuned_original<- measureFNR(pred_random$truth,
                       pred_random$response,
                       negative="0",
                       positive = "1")

fnr_ranger_tuned_original
...
```{r}
model_random_skip <- ranger::ranger(class~., data = diabetes_train_skip, classification = TRUE, probability = TRUE, num.trees=776, mtry=1, min.node.size=8, splitrule="extratrees")

samp <- sample(1:231, 5)
explainer <- explain(model = model_random_skip,
                     data = diabetes_test_skip[,-6],
             y = diabetes_test_skip$class)
