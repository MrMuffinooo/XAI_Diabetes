...
Our task was to find correlation between different biomarkers and positive diagnosis of diabetes based on the data of over 700 women of indian ancestry (Pima tribe).
...
![Figure: Distributions of features.](./images/2-4-distributions.png){width=50%}
...
During preprocessing and based on the observations of our fist models we created 4 different versions of dataset which we later used to train our models. Those versions were:

- `original` - base dataset with no modifications
 
- `naomit` - base dataset excluding observations with any missing values
 
- `skip` - base dataset excluding features `skin`, `insu` and `pres`
 
- `mice` - `skip` version with missing values imputed with mice
...
![Figure: Shape of lines resebles the nature of the model. Dot shows value and model's prediction for the observation.](./images/2-4-titanicCeterisProfile01E-1.png){width=50%}
...
Figure TODO shows Feature Importance for booth Ada and Ranger models. As we can see, booth aggree with eachother as to what variables are most important to them. Their predictions are mostly based on the values of *plas* variable and then *mass* and *age* come second.
...
Partial Dependence profile seen in figure TODO give us perhaps the most interesting results. Firstly, they are adequate to Feature importance in Figure TODO. The more important variables return wider range of average prediction and less important give less varied predictions respectively. Secondly, we can observe the difference in model's inner characteristics based on the shape of their's PDP functions. Ada is more edgy and straight while ranger more smooth. 
Based on results yielded by PDP and acording to these two models we can conclude that, generally speaking the bigger the values the higher the chance of positive Diabetes test result.
...
![Figure: Partial dependence and acumulated dependence on ranger's mass variable.](./images/2-4-pdp-ale-mass.png){width=50%}

### Local

![](./images/2-4-ceteris-paribus.png){width=50%}

![](./images/2-4-lime_190.png){width=50%}

![](./images/2-4-bd_190.png){width=50%}

![](./images/2-4-shap_190.png){width=50%}

![](./images/2-4-lime_214.png){width=50%}

![](./images/2-4-bd_214.png){width=50%}

![](./images/2-4-shap_214.png){width=50%}
...
[1] https://ema.drwhy.ai/InstanceLevelExploration.html

[2] https://www.niddk.nih.gov/health-information/diabetes/overview/symptoms-causes#causes

[3] https://www.nia.nih.gov/health/diabetes-older-people
