...
Based on the results, two models: Ranger and Ada were chosen for further analysis. Hyperparameter tuning using random grid search was performed, resulting in classificators with the following values:
...
Figure TODO shows Feature Importance for booth Ada and Ranger models. As we can see, booth agree with each other as to what variables are most important to them. Their predictions are mostly based on the values of *plas* variable and then *mass* and *age* come second.
...
Local explainations for Ada and ranger models were very simmilar so we focused on one with a bit better metrics that is Ranger. 

Ceteris Paribus shows us results adequate to Pratial Dependence Profile (See Figures TODO and TODO). Predictions are very consistent in variable _plas_. However in _age_ and _pedi_ cases we can spot different trends in prediction behaviour. This could hint thorvards potential interactions between variables. As for _mass_ and _preg_ they differ mostly in initial prediction value. Overall tendency is rising.

![Figure: Ceteris Paribus predictions for five selected observations](./images/2-4-ceteris-paribus.png){width=50%}

Let us analyze observation 190 in more detail. Lime explanation (Figure TODO) tells us three biggest factors approximated by glass box model. Having _plas_ in our case has significant negative impact on observation for having diabetes. On the other hand, having high _mass_ index and being 30 or so yers old adds to our propability.

![Figure: Lime output for observation 190](./images/2-4-lime_190.png){width=50%}

Breakdown and Shap (Figures TODO and TODO) as opposed to Lime give us prediction change for exact values, not intervals. Because of that the results may be different. So is the case with the _age_ variable which behaves differently for Lime and others. According to Breakdown and Shap being 29 years of age slightly decreases prediction but Lime says otherwise. Not only it increases but also by a significant amount. Other variables seem to mach their impact if just scaled down a bit for BD and Shap.
