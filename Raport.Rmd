...
Mashine learning is nowdays widely used in many more or less scientific fields. Some interesting ones include economy, social networking and medicine. Sometimes however we not only need a well performing model but also an understable one. The Explainable Mashine Learning (XAI) is still pretty new concept and has a lot of potential for its use. In this paper we harness XAI methods for a real world medicine problem. We take a look at the diabetic data, build a decent model based on it and try to understand what conclusions the model has reached. As a result for our experiment we will understand what factors contribute to higher chanse of diabetes occurence.

## Keywords

# Introduction (Jung)

With the rise in popularity of Machine Learning algorithms people started questioning the solutions obtained by computers' calculations. A whole new branch of AI has been formed - Explainable Artificial Intelligence (XAI).
